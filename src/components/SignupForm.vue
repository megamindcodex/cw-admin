  ...
  <div class="container mt-10">
    <v-form @submit.prevent="submitForm" class="px-3">
      <div class="form-group">
        <v-text-field type="text" placeholder="Name" variant="outlined" v-model="name" />
      </div>
      <div class="form-group">
        <v-text-field type="text" placeholder="userName" variant="outlined" v-model="userName" />
      </div>
      <div class="form-group">
      ...
      <div class="form-group pass-input">
        ...
        <i
          :class="visible ? 'fa-regular fa-eye-slash visible' : 'fa-regular fa-eye visible'"
          @click="visible = !visible"
        ></i>
      </div>
      <div class="form-group">
        <button class="pa-3" type="submit">
          <v-progress-circular v-show="isLoading" size="100" indeterminate></v-progress-circular>
          <span v-show="!isLoading">Sign up</span>
        </button>
      </div>
      <div class="mt-4">
        Already have an account?
        <RouterLink to="/login" class="text-green text-decoration-none">Login</RouterLink>
      </div>
    </v-form>
  </div>
</template>
